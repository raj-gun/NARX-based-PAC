...
axis([-pi,pi, -inf,inf, -inf,inf]); set(gca, 'FontSize', font_size); view(2); colorbar;
ylabel({'\bf HFO' ; ' ' ; '\rm High Frequency (Hz)'});
title({'NARX-based PAC';' '});
...
set(gca, 'XTick', [-pi, -0.5*pi, 0, 0.5*pi, pi]); set(gca, 'XTickLabel', {'-\pi', '-\pi/2', '0', '\pi/2', '\pi'});
set(gca, 'FontSize', font_size); view(2);
ylabel({'\bf HFO' ; ' ' ; ['\rm Mean Ampl. at ', num2str(high_freq_HFO), 'Hz']});
title({'NARX-based PAC';' '});
...
set(gca, 'FontSize', font_size); view(2);
xlabel('Low Frequency Phase (rads)');
ylabel({'\bf HG' ; ' ' ; ['\rm Mean Ampl. at ', num2str(high_freq_HG), 'Hz']});
...
imagesc(phs_bins, fhigh_MI, phs_freq ); axis xy;
set(gca, 'XTick', [-pi, -0.5*pi, 0, 0.5*pi, pi]); set(gca, 'XTickLabel', {'-\pi', '-\pi/2', '0', '\pi/2', '\pi'});
axis([-pi,pi, -inf,inf, -inf,inf]); set(gca, 'FontSize', font_size); colorbar;
title({'Filtering base';' '});
