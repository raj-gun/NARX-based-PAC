...
% u = (u - mean(u,1))./max(abs(u),1);
% u = (u - mean(u,1))./max(u,1);
% u = (u - mean(u,1))./std(u,1);
...
% y_sysid = (y_sysid - mean(y_sysid))./max(abs(y_sysid));
% y_sysid = (y_sysid - mean(y_sysid))./max(y_sysid);
% y_sysid = (y_sysid - mean(y_sysid))./std(y_sysid);
% y_sysid = ( (y_sysid-mean(y_sysid)).* ( std_cos/std(y_sysid) ) );
...
if sum(match_ind_intmod)~=0 && sum(match_ind_arx_u1)~=0 && sum(match_ind_arx_u2)~=0
    ...
    %     Y_u1_u2_phs = [ angle(hilbert(Y_u1)) , angle(hilbert(Y_u2)) ];
    %     Y_u1_u2_phs_dat = [ Y_u1_u2_phs , Y_u1_u2_phs(:,2)-Y_u1_u2_phs(:,1) , Y_u1_u2_phs(:,2)+Y_u1_u2_phs(:,1) , Y_intmd];
    %     Y_u1_u2_phs_dat = Y_u1_u2_phs_dat(1:fftn_dsrd,:);
    fftn = fftn_dsrd;
    %-------
